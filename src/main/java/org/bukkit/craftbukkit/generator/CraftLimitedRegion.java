...
public class CraftLimitedRegion extends CraftRegionAccessor implements LimitedRegion {
    ...
    private final WeakReference<net.minecraft.world.level.WorldGenLevel> weakAccess;
    private final int centerChunkX;
    private final int centerChunkZ;
    // Buffer is one chunk (16 blocks), can be seen in net.minecraft.world.level.chunk.status.ChunkStatus#q
    // there the order is {..., FEATURES, LIQUID_CARVERS, STRUCTURE_STARTS, ...}
    private final int buffer = 16;
    private final BoundingBox region;
    boolean entitiesLoaded = false;
    // Minecraft saves the entities as net.minecraft.nbt.CompoundTag during chunk generation. This causes that
    ...
    public CraftLimitedRegion(net.minecraft.world.level.WorldGenLevel access, net.minecraft.world.level.ChunkPos center) {
    ...
    public net.minecraft.world.level.WorldGenLevel getHandle() {
        net.minecraft.world.level.WorldGenLevel handle = weakAccess.get();

        Preconditions.checkState(handle != null, "net.minecraft.world.level.WorldGenLevel no longer present, are you using it in a different tick?");
    ...
    public void loadEntities() {
        ...
        net.minecraft.world.level.WorldGenLevel access = getHandle();
        // load entities which are already present
        for (int x = -(buffer >> 4); x <= (buffer >> 4); x++) {
            for (int z = -(buffer >> 4); z <= (buffer >> 4); z++) {
                net.minecraft.world.level.chunk.ProtoChunk chunk = (net.minecraft.world.level.chunk.ProtoChunk) access.getChunk(centerChunkX + x, centerChunkZ + z);
    ...
    public List<BlockState> getTileEntities() {
        ...
        for (int x = -(buffer >> 4); x <= (buffer >> 4); x++) {
            for (int z = -(buffer >> 4); z <= (buffer >> 4); z++) {
                net.minecraft.world.level.chunk.ProtoChunk chunk = (net.minecraft.world.level.chunk.ProtoChunk) getHandle().getChunk(centerChunkX + x, centerChunkZ + z);
                for (net.minecraft.core.BlockPos position : chunk.getBlockEntitiesPos()) {
