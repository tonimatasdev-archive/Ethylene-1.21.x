...
class CraftMetaPotion extends CraftMetaItem implements PotionMeta {
    ...
    static final ItemMetaKeyType<net.minecraft.world.item.alchemy.PotionContents> POTION_CONTENTS = new ItemMetaKeyType<>(net.minecraft.core.component.DataComponents.POTION_CONTENTS);
    ...
    CraftMetaPotion(net.minecraft.core.component.DataComponentPatch tag) {
        super(tag);
        getOrEmpty(tag, POTION_CONTENTS).ifPresent((potionContents) -> {
            ...
            List<net.minecraft.world.effect.MobEffectInstance> list = potionContents.customEffects();
            int length = list.size();
            customEffects = new ArrayList<>(length);

            for (int i = 0; i < length; i++) {
                net.minecraft.world.effect.MobEffectInstance effect = list.get(i);
    ...
    void applyToItem(CraftMetaItem.Applicator tag) {
        ...
        List<net.minecraft.world.effect.MobEffectInstance> effectList = new ArrayList<>();
        if (customEffects != null) {
            for (PotionEffect effect : customEffects) {
                effectList.add(new net.minecraft.world.effect.MobEffectInstance(CraftPotionEffectType.bukkitToMinecraftHolder(effect.getType()), effect.getDuration(), effect.getAmplifier(), effect.isAmbient(), effect.hasParticles(), effect.hasIcon()));
            }
        }

        tag.put(POTION_CONTENTS, new net.minecraft.world.item.alchemy.PotionContents(defaultPotion, potionColor, effectList));
    }
