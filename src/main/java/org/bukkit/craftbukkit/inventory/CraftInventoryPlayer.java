...
public class CraftInventoryPlayer extends CraftInventory implements org.bukkit.inventory.PlayerInventory, EntityEquipment {
    ...
    public void setItem(int index, ItemStack item) {
        ...
        net.minecraft.server.level.ServerPlayer player = ((CraftPlayer) this.getHolder()).getHandle();
        if (player.connection == null) return;
        // net.minecraft.network.protocol.game.ClientboundContainerSetSlotPacket places the items differently than setItem()
        //
        // Between, and including, index 9 (the first index outside of the hotbar) and index 35 (the last index before
        // armor slots) both net.minecraft.network.protocol.game.ClientboundContainerSetSlotPacket and setItem() places the items in the player's inventory the same way.
        ...
        //
        // net.minecraft.network.protocol.game.ClientboundContainerSetSlotPacket works very differently. Slots 0-8 are as follows: 0 crafting output, 1-4 crafting input,
        // 5 helmet, 6 chestplate, 7 leggings, and 8 boots. Then, 9-35 work exactly the same as setItem(). The hotbar
        // for net.minecraft.network.protocol.game.ClientboundContainerSetSlotPacket starts at index 36, and continues to index 44. Items placed where index is < 0 or
        // > 44 have no action. Basically, the upper part of the player's inventory (crafting area and armor slots) is
        // the first "row" of 9 slots for net.minecraft.network.protocol.game.ClientboundContainerSetSlotPacket. From there the rows work as normal, from left to right
        // all the way down, including the hotbar.
        //
        // With this in mind, we have to modify the index we give net.minecraft.network.protocol.game.ClientboundContainerSetSlotPacket to match the index we intended
        ...
        player.connection.send(new net.minecraft.network.protocol.game.ClientboundContainerSetSlotPacket(player.inventoryMenu.containerId, player.inventoryMenu.incrementStateId(), index, CraftItemStack.asNMSCopy(item)));
    }
    ...
    public void setHeldItemSlot(int slot) {
        ...
        ((CraftPlayer) this.getHolder()).getHandle().connection.send(new net.minecraft.network.protocol.game.ClientboundSetCarriedItemPacket(slot));
    }
    ...
    public void setItemInMainHandDropChance(float chance) {
        throw new UnsupportedOperationException("Cannot set drop chance for net.minecraft.world.entity.player.Inventory");
    }
